...
@lru_cache(maxsize=1)
def load_rewrite_rules() -> dict[str, RewriteRule]:
    resource = resources.files("hangeulint").joinpath(
        "references", "rewrite-rules.json"
    )
    payload: dict[str, Any] = json.loads(resource.read_text(encoding="utf-8"))
    if payload.get("schema_version") != "0.1":
        raise ValueError("지원하지 않는 rewrite rule registry schema입니다.")
    evidence = load_evidence()
    rules: dict[str, RewriteRule] = {}
    for index, item in enumerate(payload.get("rules", [])):
        rule_id = item.get("id")
        if not isinstance(rule_id, str) or not rule_id:
            raise ValueError(f"rewrite rules[{index}].id가 필요합니다.")
        if rule_id in rules:
            raise ValueError(f"중복 rewrite rule id: {rule_id}")
    ...
    if not rules:
        raise ValueError("rewrite rule registry가 비어 있습니다.")
    return rules


def get_rewrite_rule(rule_id: str) -> RewriteRule:
    try:
        return load_rewrite_rules()[rule_id]
    except KeyError as exc:
        raise ValueError(f"알 수 없는 rewrite rule id: {rule_id}") from exc
